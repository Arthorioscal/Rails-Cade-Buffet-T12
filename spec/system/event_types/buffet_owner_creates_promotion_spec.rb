...
describe 'Buffet owner creates promotion' do
    ...
    it 'and must fill all fields' do
        ...
        fill_in 'Desconto para dias de semana', with: ''
        fill_in 'Desconto para finais de semana', with: ''
        click_on 'Criar Promoção'

        expect(page).to have_content('Data de Início não pode ficar em branco')
        expect(page).to have_content('Data de Término não pode ficar em branco')
        expect(page).to have_content('Desconto para dias de semana não pode ficar em branco')
        expect(page).to have_content('Desconto para finais de semana não pode ficar em branco')
        expect(page).to have_content('Desconto para dias de semana não é um número')
        expect(page).to have_content('Desconto para finais de semana não é um número')
    end

    it 'and let name blank' do
        user = User.create!(email: 'raiden@mgs.com', password: 'solidsnakefan', role: :buffet_owner)
        buffet = Buffet.create!(brand_name: 'Buffet do Snake', corporate_name: 'Buffet do Snake Ltda', cnpj: '45195101000101',
            phone: '11999999999', email: 'solidsnakefans@mail.com', address: 'Rua do Buffet, 123', neighborhood: 'Bairro do Buffet',
            state: 'São Paulo', city: 'São Paulo', zip_code: '12345678', description: 'Buffet especializado em festas de aniversário',
