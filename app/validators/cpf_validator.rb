class CpfValidator < ActiveModel::EachValidator
    ...
    def first_digit_valid?(cpf_numbers)
      ...
      multiplied = first_digits.map.with_index { |number, index| number * (10 - index) }
      mod = multiplied.reduce(:+) % 11
      fst_verifier_digit = (11 - mod) % 10
      fst_verifier_digit == cpf_numbers[9]
    end
  
    def second_digit_valid?(cpf_numbers)
      second_digits = cpf_numbers[0..9]
      multiplied = second_digits.map.with_index { |number, index| number * (11 - index) }
      mod = multiplied.reduce(:+) % 11
      snd_verifier_digit = (11 - mod) % 10
